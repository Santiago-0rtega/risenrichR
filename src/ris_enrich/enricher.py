...
# The minimum similarity ratio (0.0 to 1.0) required between the search title 
# and the API result title to accept the abstract. Protects against false positives.
SIMILARITY_THRESHOLD = 0.80  

# Email required by Crossref's "Polite Pool" for API access.
CROSSREF_EMAIL = "esantos2@ualberta.ca"
...
def fetch_crossref(title=None, doi=None):
    """Queries the Crossref API, safely encoding international titles."""
    base_url = "https://api.crossref.org/works"
    headers = {'User-Agent': f'AbstractEnricher/1.0 (mailto:{CROSSREF_EMAIL})'}
...
def enrich_ris_file(input_path, output_path):
    """
    ...
    for i, entry in enumerate(entries):
        ...
        if title or doi:
            ...
            if not new_abstract:
                new_abstract, source = fetch_crossref(title, doi), "Crossref"
